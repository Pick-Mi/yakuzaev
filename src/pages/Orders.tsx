...
const Orders = () => {
      ...
      const ordersWithProducts = await Promise.all((data || []).map(async order => {
        ...
        if (orderItemsData && Array.isArray(orderItemsData) && orderItemsData.length > 0) {
          ...
          if (productId) {
            const {
              data: productData
            } = await supabase.from('products').select('thumbnail, image_url, images, name, variants, color_variety').eq('id', productId).maybeSingle();
            if (productData) {
              // Add product thumbnail to order_items_data
              order.order_items_data[0].image_url = productData.thumbnail || productData.image_url || productData.images && productData.images[0]?.url;
              ...
              if (productData.color_variety && colorName) {
                ...
                if (colorVariety.colors && Array.isArray(colorVariety.colors)) {
                  const colorMatch = colorVariety.colors.find((c: any) => c.name?.toLowerCase() === colorName.toLowerCase());
  ...
  const applyFilters = () => {
    ...
    // Apply search filter
    if (searchQuery) {
      filtered = filtered.filter(order => order.id.toLowerCase().includes(searchQuery.toLowerCase()) || order.order_number && order.order_number.toString().includes(searchQuery) || order.order_items_data && JSON.stringify(order.order_items_data).toLowerCase().includes(searchQuery.toLowerCase()));
    }

    // Apply status filters
    const activeStatusFilters = Object.entries(statusFilters).filter(([_, isActive]) => isActive).map(([key]) => {
      switch (key) {
        case 'onTheWay':
          return ['shipped', 'processing', 'pending', 'confirmed'];
        case 'delivered':
          return ['delivered'];
        case 'cancelled':
          return ['cancelled'];
    ...
    if (activeStatusFilters.length > 0) {
      filtered = filtered.filter(order => activeStatusFilters.includes(order.status.toLowerCase()));
    }

    // Apply time filters
    const now = new Date();
    const activeTimeFilters = Object.entries(timeFilters).filter(([_, isActive]) => isActive).map(([key]) => key);
      ...
      filtered = filtered.filter(order => {
        ...
        return activeTimeFilters.some(filter => {
          switch (filter) {
            ...
            case 'year2024':
              return orderYear === 2024;
            case 'year2023':
              return orderYear === 2023;
            case 'year2022':
              return orderYear === 2022;
            case 'year2021':
              return orderYear === 2021;
            case 'older':
              return orderYear < 2021;
            default:
              return false;
  ...
  const getPaymentStatusBadge = (paymentStatus?: string) => {
    ...
    return <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium ${isPaid ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-orange-100 text-orange-700 border border-orange-200'}`}>
        {isPaid ? <>
  ...
  return <div className="min-h-screen bg-white">
      ...
      <div className="space-y-0">
            ...
            <div className="bg-white mt-[52px] px-4 py-6 space-y-6">
              {filteredOrders.length === 0 ? <div className="text-center py-12 px-4 bg-white rounded-lg">
                  <Package className="w-16 h-16 mx-auto text-gray-400 mb-4" />
                  <h2 className="text-xl font-semibold mb-2">
                    {orders.length === 0 ? "No orders yet" : "No orders found"}
                  </h2>
                  <p className="text-gray-600 mb-4">
                    {orders.length === 0 ? "You haven't placed any orders yet." : "Try adjusting your filters or search query."}
                  </p>
                  {orders.length === 0 && <Button onClick={() => navigate('/')} className="bg-orange-500 hover:bg-orange-600">
                      Start Shopping
                    </Button>}
                </div> : <>
              ...
              return <div key={order.id} className="bg-white rounded-lg p-4">
                        ...
                        {/* Product Card */}
                        <button onClick={() => handleViewOrder(order.id)} className="w-full flex gap-4 items-start text-left">
                          {/* Product Image */}
                          <div className="w-[188px] h-[188px] flex-shrink-0 bg-gray-100 rounded-lg overflow-hidden">
                            {firstItem?.image_url ? <img src={firstItem.image_url} alt={firstItem.name || "Product"} className="w-full h-full object-cover" /> : <div className="w-full h-full flex items-center justify-center">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center gap-2 text-base">
                              ...
                              <div className="w-8 h-8 border border-gray-300 rounded" style={{
      ...
      <div className="flex gap-8">
            ...
            <aside className="w-64 flex-shrink-0">
              ...
              <nav className="space-y-1">
                <button onClick={() => navigate("/profile?section=profile")} className="w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-gray-50">
                  <User className="w-5 h-5" />
                  <span>Profile Details</span>
                </button>
                <button onClick={() => navigate("/profile?section=address")} className="w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-gray-50">
                  <MapPin className="w-5 h-5" />
                  <span>Delivery Addresses</span>
                </button>
                <button onClick={() => navigate("/profile?section=pan")} className="w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-gray-50">
                  <CreditCard className="w-5 h-5" />
                  <span>Identification Details</span>
                </button>
                <button onClick={() => navigate("/orders")} className="w-full flex items-center gap-3 px-4 py-3 text-left transition-colors bg-white border-l-4 border-orange-500 font-medium">
                  <ShoppingCart className="w-5 h-5" />
                  <span>My Orders</span>
                </button>
                <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-gray-50">
            ...
            <main className="flex-1">
              ...
              {filteredOrders.length === 0 ? <div className="text-center py-12">
                  ...
                  {orders.length === 0 && <Button onClick={() => navigate('/')}>
                      Start Shopping
                    </Button>}
              ...
              const firstItem = orderItems[0];
              const deliveryDate = order.estimated_delivery_date || order.created_at;
              return <div key={order.id} className="border border-gray-200 p-6">
                        {/* Delivery Date Header */}
                        <div className="mb-4">
                        ...
                        <div className="flex gap-6">
                          ...
                          <div className="flex-1">
                            ...
                            {firstItem?.variant && <p className="text-sm text-gray-700 mb-1">
                        ...
                        backgroundColor: firstItem?.color_hex || '#000000'
                          ...
                          maximumFractionDigits: 2
                            ...
                            <Button onClick={() => handleViewOrder(order.id)} className="bg-black text-white hover:bg-gray-800 px-8 rounded-none">
                              View Order
                            </Button>
                          </div>
                        </div>
